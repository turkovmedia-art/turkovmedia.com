...
function renderPortfolioGrid(filter = 'all') {
    ...
    filteredProjects.forEach(project => {
            ...
            <div class="portfolio-thumb-box">
                ...
                <div class="portfolio-overlay">
                    <div class="play-trigger">
                        <video class="hover-logo-video" muted playsinline preload="none" style="width: 100%; height: 100%; object-fit: contain; pointer-events: none;">
        ...
        
        {
            ...
            // preload is "none" (every card used to eagerly download this same clip on page
            // load - 44 cards x 243KB was ~11MB downloaded before anyone touched anything).
            // Fetch it for real only the first time this specific card is actually interacted
            // with; after that the browser's own HTTP cache serves every other card's request
            // for the identical URL, so this still costs one real download for the whole page.
            const ensureVideoLoaded = () => {
                if (!video || video.dataset.loadTriggered) return;
                video.dataset.loadTriggered = 'true';
                video.load();
            };
...
// The travelling glow around the contact form (style.css) animates a custom property
// (--contact-glow-angle) registered via @property, which is what lets the browser interpolate
// it smoothly. Browsers without @property support (older Firefox, older Safari, etc.) still
// render the conic-gradient - it just cannot animate, so it sits frozen at its 0deg fallback
// forever ("the effect is there, it just never moves"). Detect that case and drive the same
// property by hand with requestAnimationFrame instead - same 12s per revolution, same
// prefers-reduced-motion respect, just interpolated in JS rather than by the CSS engine.
function initContactGlowFallback() {
    const supportsAnimatableProperty = typeof CSS !== 'undefined' && typeof CSS.registerProperty === 'function';
    if (supportsAnimatableProperty) return;

    const card = document.querySelector('.contact-form-box');
    if (!card) return;
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

    const DEGREES_PER_MS = 360 / 12000; // full revolution every 12s, matching contact-glow-spin
    let start = null;

    const tick = (timestamp) => {
        if (start === null) start = timestamp;
        const angle = ((timestamp - start) * DEGREES_PER_MS) % 360;
        card.style.setProperty('--contact-glow-angle', angle + 'deg');
